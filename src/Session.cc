...
Session::Session() {
  ...
  if(vm.count("sqlite")) {
    if(!vm.count("sqlite-database")) {
      std::cout << "ERROR: You have to specify the file name for the sqlite data base (sqlite-database option)"
                << std::endl;
      exit(1);
    }
    theDB = std::make_unique<Dbo::backend::Sqlite3>(
        WApplication::instance()->appRoot() + vm["sqlite-database"].as<std::string>());
  }
  else {
    if(!vm.count("mysql-database") || !vm.count("mysql-user") || !vm.count("mysql-password") ||
        !vm.count("mysql-host")) {
  ...
  try {
    ...
    log("info") << "Presumably the database already exists, since creating resulted in the following error: "
                << e.what();
...
void Session::registerUser(std::string login, std::string email, Wt::WString password) {
...
void Session::updateUser(Wt::Dbo::ptr<User> user, std::string email, Wt::WString password) {
  // add User data structure and set user role to Admin
